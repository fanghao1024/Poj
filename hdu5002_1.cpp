...
struct Linked_Cut_Tree{
    int fa[max_n],son[max_n][2],st[max_n],num[max_n],ta[max_n],tc[max_n],siz[max_n],top;
    int mx1[max_n],c1[max_n],mx2[max_n],c2[max_n];
    bool rev[max_n];
    void init(){
        for(int i=1;i<=N;i++){
    ...
    void cut(int x,int y){
        ...
        if(son[y][0]==x&&!son[x][1]) 
            son[y][0]=fa[son[y][0]]=0,update(y);
    }
    void query(int x){
        if(c1[x]==siz[x]) printf("ALL SAME\n");
        else printf("%d %d\n",mx2[x],c2[x]);
    }
}LCT;
int main(){
    ...
    for(int case_no=1;case_no<=T;case_no++){
        ...
        for(int i=1;i<N;i++) scanf("%d %d",&a,&b),LCT.link(a,b);
        while(M--){
            scanf("%d",&ops);
            switch(ops){
                case 1:
