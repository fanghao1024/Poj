...
struct Tr{
    ...
    void insert(int& p,int val){
        ...
        node[p].size++;
        if(node[p].val==val){node[p].ct++;return ;}
        bool c=node[p].val<val; // 加入右节点为1
        insert(node[p].son[c],val);
        update(p);
        if(node[node[p].son[c]].pri>node[p].pri) rotate(p,c);
    }
    void del(int& p,int val){
        if(!p) return ;
        if(node[p].val==val){
            if(node[p].ct>1){node[p].ct--;node[p].size--;return ;}
    ...
    int rank(int p,int val){
        if(!p) return 0;
        if(node[p].val==val) return node[lson].size;
        else if(node[p].val>val) return rank(lson,val);
        else return rank(rson,val)+node[lson].size+node[p].ct;
    }
...
int queryRank(int L,int R,int val,int p,int pl,int pr){
    ...
    if(L<=pl&&pr<=R) return tr[p].rank(tr[p].root,val);
